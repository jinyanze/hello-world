...
<template>
	<view class="uni-select-cy" :style="{'z-index':zindex}">
		...
		<scroll-view class="uni-select-cy-options" :scroll-y="true" v-show="active" @scrolltolower="scrolltolower">
			<template>
				<view class="uni-select-cy-item" :class="{ active: realValue.includes(item[slabel]) }"
					v-for="(item, index) in options" :key="index" @click.stop="handleChange(listIndex,index, item)">
					{{ item[slabel] }}
				</view>
			</template>
		</scroll-view>
...
<script>
	export default {
		...
		methods: {
			...
			//点击组件列
			handleChange(listIndex,index, item) {
				let arrIndex = this.realValue.indexOf(item[this.slabel]);
				if (arrIndex > -1) {
					this.changevalue.splice(arrIndex, 1);
					this.realValue.splice(arrIndex, 1);
				} else {
					this.changevalue.push(item);
					this.realValue.push(item[this.slabel]);
				}
				console.log(this.realValue, 'this.realValue')
				this.$emit('change', listIndex,this.changevalue, this.realValue);
			}
		}
	};
</script>
	...
	export default {
		...
		methods: {
			...
			handleChange(index, item) {
				...
				console.log('aaa', this.realValue);
				this.$emit('change', this.changevalue, this.realValue, this.listIndex);
			}
		}
	};
</script>
